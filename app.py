...
def get_title(predictions, categories=CAT6):
    ...
    mental_states_str = ",".join(mental_state) if mental_state else "None"
...
def main():
    ...
    if choice == "Upload audio":
        ...
        if audio_file is not None:
            ...
            # ***********************************************
            data_fin, sample_rate_fin = librosa.load(OPEN("WEB-UI","rb"+str(path)))
            data,sample_rate=data_fin,sample_rate_fin
            X = []

            feature = get_features(open("WEB-UI","rb"+str(path)))
            for ele in feature:
                X.append(ele)
